...
if(isset($_GET['id']) && intval($_GET['id']) > 0){
    $fid = intval($_GET['id']);
    $qry = $conn->query("SELECT * from `fee_list` where id = '{$fid}' ");
    if($qry->num_rows > 0){
	...
	<form action="" id="fee-form">
		...
		<div class="form-group">
			<div class="form-check">
				<input class="form-check-input" type="checkbox" value="1" id="status" name="status" <?php echo isset($status) && $status == 1 ? 'checked' : ( !isset($status) ? 'checked' : '' ) ?> />
...
<script>
	...
	$(document).ready(function(){
		...
		$('#fee-form').submit(function(e){
			...
			start_loader();
			$.ajax({
				url:_base_url_+"classes/Master_api.php?f=save_fee",
