...
<body class="login-page dark-mode py-4">
  ...
  <style>
    ...
    #user-register .form-control { font-size: 16px; }
    #user-register label.control-label { font-size: 14px; margin-bottom: 5px; font-weight: 500; }
  </style>
  <h1 class="text-center py-5 login-title"><b><?php echo $_settings->info('name') ?></b></h1>
<div class="login-box">
    <div class="card card-primary card-outline card-tabs glass-card">
    ...
    $('#user-register').submit(function(e){
        ...
        $.ajax({
            ...
            error: function(err) {
                ...
                end_loader();
                var msg = 'Registration failed due to a network error. Please try again.';
            ...
            success:function(resp){
                ...
                // For any other response (including empty or unexpected responses)
                    ...
                    end_loader();
                    el.addClass('alert-danger')
                    el.text('An unexpected error occurred during registration. Please try again.')
                    _this.prepend(el)
                    el.show('slow')
                    $('html,body').animate({scrollTop:0},'fast')
                }
